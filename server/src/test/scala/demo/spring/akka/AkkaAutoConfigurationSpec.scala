...
class AkkaAutoConfigurationSpec extends FlatSpec with TestContextManagement with Matchers with AskSupport with ScalaFutures with SpringLogging {

  import demo.spring.akka.AkkaAutoConfigurationSpec.KeyValueProtocol._
...
object AkkaAutoConfigurationSpec {
   ...
   */
  @ActorComponent
  class ForwardingActor(nextActor: ActorRef) extends Actor with SpringLogging {
  ...
  @Component
  class KeyValueStore extends collection.mutable.HashMap[String, String] with SpringLogging
   ...
   */
  @ActorComponent
  class KeyValueActor @Autowired()(val store: KeyValueStore) extends Actor with SpringLogging {
